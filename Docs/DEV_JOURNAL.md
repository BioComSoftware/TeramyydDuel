...
Crew HUD drag/drop fully operational. All crew assignment scenarios working: unassigned→station, station→station, station→unassigned. System uses pre-emptive visual updates + drag state tracking + immediate persistence to prevent race conditions with refresh cycle.

### Key Components & Data Flow

**CrewHUDController.cs**
- Central coordinator for icon lifecycle, periodic refresh (0.4s interval), drag handling
- `HandleStationDrop(slot, icon)`: Pre-emptively attaches icon to slot, calls CrewManager.TryAssignCrewToStationId, returns bool success
- `HandleReturnToPool(icon)`: Immediately attaches icon to pool, calls CrewManager.UnassignCrew with forceSave=true, suppresses refresh for 0.5s
- `RefreshAssignments()`: Skips icons with IsDragging=true, applies crew.AssignedStation state to visual positioning
- `ApplyCrewAssignment(icon, crew)`: Determines icon placement based on crew.AssignedStation (null=pool, valid=station slot)
- Suppression timer (`_suppressRefreshUntil`) prevents refresh from interfering with manual drag operations

**CrewHUDCrewIcon.cs**
- Individual crew portrait with drag/drop handlers, tooltip integration
- `IsDragging` property: Set true in OnBeginDrag, false in OnEndDrag—prevents refresh from moving icon during drag
- `EnsureDropHandled()` coroutine: Uses WaitForEndOfFrame, calls HandleReturnToPool if no drop zone claimed the icon
- `NotifyDropHandled()`: Called by drop zones (CrewHUDStationSlot, CrewHUDUnassignedZone) to prevent fallback processing

**CrewHUDStationSlot.cs**
- Drop target for specific CrewStation, manages icon anchoring
- `station` field: Direct reference to CrewStation component (required, no override)
- `StationId` property: Returns station.stationId
- `OnDrop()`: Calls controller.HandleStationDrop, notifies icon, snaps back on failure
- Removed `stationIdOverride`—all slots now require direct CrewStation reference

**CrewManager.cs**
- Backend authority for crew-station relationships, persistence coordination
- `TryAssignCrewToStationId(crew, stationId)`: Looks up station in _stationsById dict, fallback searches scene via FindObjectsOfType
- `TryAssignCrewToStation(crew, station)`: Checks station.CanAssign(), calls RemoveFromCurrentStation + station.AddCrewInternal, updates persistence
- `UnassignCrew(crew)`: Calls RemoveFromCurrentStation, forces immediate persistence save
- `RegisterStation(station)`: Called by CrewStation.OnEnable and CrewManager.Start, warns on duplicate IDs
- `Start()`: Finds all CrewStation components in scene via FindObjectsOfType(includeInactive), registers active ones

**CrewStation.cs**
- Logical crew assignment point on ships (weapons, engines, lift devices)
- `stationId`: Unique identifier, generated by parent component (WeaponMount, Engine, LiftDevice) or GUID fallback
- `OnEnable()`: Registers self with CrewManager.Instance
- `AddCrewInternal(member)`: Sets member.AssignedStation = this, clears PendingStationId
- `RemoveCrewInternal(member)`: Sets member.AssignedStation = null if matched

**WeaponMount.cs, Engine.cs, LiftDevice.cs**
- Auto-create CrewStation components at runtime if missing (autoCreateCrewStation=true)
- `EnsureCrewStation()`: Sets station.stationId based on GameObject name to ensure uniqueness
  - WeaponMount: `{gameObject.name}_crew_slot` (e.g., "Bow_weapon_mount_crew_slot")
  - Engine: `{gameObject.name}_EngineCrew`
  - LiftDevice: `{gameObject.name}_LiftCrew`
- Force-regenerates station IDs to prevent duplicate registrations (multiple mounts with same mountId)

**CrewPersistenceManager.cs**
- JSON serialization to Assets/Resources/CrewPersistence.json
- `UpdateCrewAssignment(crewId, stationId, forceSave)`: Updates in-memory state, optionally saves immediately
- forceSave=true used by UnassignCrew to prevent stale reads during refresh cycles

### Station ID Uniqueness (Critical)
**Problem:** Multiple stations registering with same ID (e.g., all weapon mounts using "Mount_01_crew_slot")
**Solution:** Use GameObject.name instead of mountId for station ID generation
**Why:** mountId often duplicated across mounts (all set to "Mount_01"), GameObject names are unique in hierarchy
**Registration Warning:** CrewManager logs warning if duplicate station ID detected, helps diagnose inspector configuration errors

### Unity Scene Wiring Requirements
1. **CrewHUDStationSlot.station**: Must reference actual CrewStation component on weapon mount/engine/lift device GameObject
2. **CrewHUDStationSlot.iconAnchor**: RectTransform where crew icon snaps to (UI space)
3. **CrewHUDStationSlot.worldAnchor**: Transform in 3D space for future crew model placement (e.g., "Bow_weapon_mount_CrewAnchor")
4. **CrewStation components**: Auto-created at runtime by WeaponMount/Engine/LiftDevice, or manually added
5. **Unique GameObject names**: Each weapon mount/engine/lift device must have unique name in hierarchy (not mountId)

### Verified Behaviors (All Working)
1. ✅ Startup: Crew with assignedStationId in persistence appear at station slots, others in unassigned anchors
2. ✅ Unassigned→Station: Drag from pool to station, backend assigns, persistence saves, 3D crew spawns
3. ✅ Station→Station: Direct reassignment, updates persistence, moves 3D crew
4. ✅ Station→Unassigned: Drag to empty space/unassigned zone, backend unassigns, returns to pool
5. ✅ Station capacity limits: CanAssign() prevents over-assignment, logs queued pending
6. ✅ Refresh cycle coexistence: Drag state tracking + suppression timer prevent race conditions

### Logging System
All crew operations log to both Debug.Log (console) and FileLogger (Logs/game_debug.log) with [CrewHUD], [CrewManager], [CrewStation] prefixes for filtering. Comprehensive diagnostic coverage for assignment flow, registration, drop handling.

## 2025-12-06 — Crew HUD Drag Pipeline Overhaul (Historical)
